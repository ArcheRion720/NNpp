...
void Network::File::read(const char* fname, Network& nn)
{	
	...
	if (!header.valid())
		throw Network::File::InvalidHeaderException();

	size_t* layers = new size_t[header.layers];
	...
	nn.init(layers, header.layers, header.lr, header.gamma);
...
inline void Network::File::readMatrix(Matrix* mat, double* buffer, std::ifstream& file)
{
...
void Network::File::write(const char* fname, Network& nn)
{
	NPPFileHeader header;
...
inline void Network::File::writeMatrix(Matrix& mat, std::ofstream& file)
{
